...
import type { CalcInput, SynastryResponse, SoulmateSignalsResponse, ZodiacElement } from '../types';
...
export function buildSynastryResponse(a: CalcInput, b: CalcInput): SynastryResponse {
  ...
  const emotional = clampScore(50 + Math.round(emotionalScore / 8));

  // 计算沟通契合度（水星相关相位）
  const communicationScore = crossAspects
    .filter((a1) => a1.from === 'Mercury' || a1.to === 'Mercury')
    .reduce((acc, a1) => acc + a1.score, 0);
  const communication = clampScore(50 + Math.round(communicationScore / 8));

  // 计算长期关系契合度（土星、木星相关相位）
  const longTermScore = crossAspects
    .filter((a1) => a1.from === 'Saturn' || a1.to === 'Saturn' || a1.from === 'Jupiter' || a1.to === 'Jupiter')
    .reduce((acc, a1) => acc + a1.score, 0);
  const longTerm = clampScore(50 + Math.round(longTermScore / 8));
 ...
 * @returns 宫位编号（1-12）
 */
function getHouseIndex(longitude: number, cusps: number[], ascendant: number): number {
