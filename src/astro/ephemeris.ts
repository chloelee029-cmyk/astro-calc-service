...
import { calculateAspectScore, detectAspect, moonPhaseFromAngle } from './aspects';
import { normalizeAngle, round } from '../utils/math';
import { addDays, formatIsoDate } from '../utils/date';

export function dateAtUtcNoon(date: Date): Date {
...
export function buildTransit(input: CalcInput, date: Date): NatalChartResponse {
  return buildNatalChartResponse({
    ...input,
    birthTimeISO: dateAtUtcNoon(date).toISOString(),
  });
}

export function planetMap(chart: NatalChartResponse) {
  return Object.fromEntries(chart.planets.map((planet) => [planet.planet, planet])) as Record<PlanetName, typeof chart.planets[0]>;
}

export function moonPhase(chart: NatalChartResponse) {
  const byPlanet = planetMap(chart);
  const angle = round(normalizeAngle(byPlanet.Moon.longitude - byPlanet.Sun.longitude), 2);
  return { name: moonPhaseFromAngle(angle), angle };
}

// 单日 planetary weather 是“当前天空”的纯计算快照。
export function planetaryWeather(date: Date, chart: NatalChartResponse): PlanetaryWeather {
  return {
...
function eventSeverity(planet: PlanetName): 'low' | 'medium' | 'high' {
  if (planet === 'Mercury' || planet === 'Jupiter' || planet === 'Saturn') return 'high';
  if (planet === 'Venus' || planet === 'Mars' || planet === 'Uranus' || planet === 'Neptune' || planet === 'Pluto') return 'medium';
  return 'low';
}

function dedupeEvents(events: CriticalEvent[]): CriticalEvent[] {
  const byKey = new Map<string, CriticalEvent>();
...
const COLLECTIVE_ASPECT_CANDIDATES: Array<{
...
function buildCollectiveAspectKey(source: PlanetName, aspectType: string, target: PlanetName): string {
  return `${source.toLowerCase()}_${aspectType.toLowerCase()}_${target.toLowerCase()}`;
}

// 大环境相位只看“天上行星彼此之间”的关系，不混入用户本命盘。
function scanCollectiveAspects(weather: PlanetaryWeather[]): AspectDetail[] {
  ...
  for (const dayWeather of weather) {
    ...
    for (const candidate of COLLECTIVE_ASPECT_CANDIDATES) {
      ...
      const aspect = detectAspect(source.longitude, target.longitude);
      if (!aspect) continue;
...
export function buildGlobalEventsForInput(input: CalcInput, start: Date, end: Date): {
  events: CriticalEvent[];
  weather: PlanetaryWeather[];
  collectiveAspects: AspectDetail[];
  calculation_meta?: NatalChartResponse['calculation_meta'];
} {
  const days = dateRange(start, end);
  const events: CriticalEvent[] = [];
  const weather: PlanetaryWeather[] = [];
  let previous: NatalChartResponse | null = null;
  let calculationMeta: NatalChartResponse['calculation_meta'];
...
export function buildPlanetaryPositionsForInput(input: CalcInput, date: Date): PlanetaryWeather & {
  calculation_meta?: NatalChartResponse['calculation_meta'];
} {
