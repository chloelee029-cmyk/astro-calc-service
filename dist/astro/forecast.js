...
function aspectPriority(body, aspect, minimumOrb, repeatCount) {
    ...
    return (0, math_1.clampScore)(42 + transitPlanetWeight(body) + aspectWeight + orbBonus + Math.min(5, repeatCount * 2));
}
function eventScoreForAspect(body, targetName, aspect, minimumOrb, orbLimit, repeatCount) {
    ...
    return (0, math_1.clampScore)(30 + transitPlanetWeight(body) + aspectWeight + exactnessBonus + targetBonus + repeatBonus);
}
function normalizePriority(raw, minRaw, maxRaw, minScore = 58, maxScore = 97) {
    if (maxRaw <= minRaw)
...
function categoriesForAspect(area, targetName) {
    const categories = new Set([area]);
    if (targetName === 'Venus') {
    ...
    if (targetName === 'Saturn' || targetName === 'MC')
        categories.add('career');
    if (targetName === 'DSC')
        categories.add('relationships');
    if (targetName === 'IC')
        categories.add('home_family');
...
function importanceForIntensity(intensity) {
    if (intensity >= 90)
        return 5;
    if (intensity >= 80)
        return 4;
...
function importanceForHouseChange(body) {
    if (body === 'Saturn' || body === 'Pluto')
        return 4;
    if (body === 'Jupiter' || body === 'Uranus' || body === 'Neptune')
        return 3;
    return 2;
}
function eventScoreForHouseChange(body) {
    if (body === 'Saturn' || body === 'Pluto')
        return 82;
    if (body === 'Jupiter' || body === 'Uranus' || body === 'Neptune')
        return 72;
...
function monthWindow(month) {
    ...
    return { startDate: (0, date_1.formatIsoDate)(start), endExclusive: (0, date_1.formatIsoDate)(end) };
}
...
function buildMonthlyTimeline(args) {
    return monthKeys(args.start).map((month) => {
        ...
        const exactAspectIds = new Set();
        const exactAspects = args.aspects
            .filter((aspect) => aspect.peakDate.startsWith(month))
            .sort((a, b) => b.priority - a.priority);
        for (const aspect of exactAspects)
            exactAspectIds.add(aspect.id);
...
function confidenceForWindow(aspect) {
    ...
    return (0, math_1.round)(Math.min(0.95, orbConfidence + repeatBonus - 0.05), 2);
}
function overallYearTone(aspects) {
    ...
    const pressureScore = aspects
        ...
        .reduce((sum, aspect) => sum + aspect.priority, 0);
    if (growthScore > pressureScore * 1.25)
        return 'growth';
    if (pressureScore > growthScore * 1.25)
        return 'pressure';
    if (growthScore > 0 && pressureScore > 0)
        return 'mixed';
    return 'integration';
}
function buildOpportunityWindows(aspects, type, themes = []) {
    ...
    if (type === 'caution' || aspectWindows.length >= 5)
        return aspectWindows;
    ...
    const themeWindows = themes
        ...
        confidence: (0, math_1.round)(Math.min(0.86, 0.68 + theme.evidenceIds.length * 0.02), 2),
    }));
...
function buildAiEvidenceSummary(args) {
    ...
    const houseChangesByKey = new Map();
    for (const placement of args.houses) {
    ...
    return {
        ...
        overallIntensity: args.aspects.length
            ? (0, math_1.clampScore)(Math.round(args.aspects.slice(0, 8).reduce((sum, aspect) => sum + aspect.priority, 0) / Math.min(8, args.aspects.length)))
            : 50,
