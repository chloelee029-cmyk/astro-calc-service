...
exports.aspectWeight = aspectWeight;
 ...
 */
function detectAspect(a, b, orbLimit = constants_1.ASPECT_ORB_LIMIT) {
    const delta = (0, math_1.angleDistance)(a, b);
    let winner = null;
    // 遍历所有相位类型，寻找最接近且在容许度内的相位
    for (const [type, angle] of Object.entries(constants_1.ASPECT_ANGLES)) {
        const orb = Math.abs(delta - angle);
        if (orb <= orbLimit && (!winner || orb < winner.orb)) {
            winner = { type: type, orb };
        }
    }
    return winner;
}
 ...
 */
function aspectWeight(type) {
    return constants_1.ASPECT_WEIGHTS[type] || 0;
}
/**
 * 根据日月角度计算月相名称
 * @param angle - 日月黄经差
 ...
 * @returns 共振强度（0-1）
 */
function calculateResonance(orb, orbLimit = constants_1.ASPECT_ORB_LIMIT) {
 ...
 * @returns 相位分数
 */
function calculateAspectScore(type, orb, orbLimit = constants_1.ASPECT_ORB_LIMIT) {
 ...
 * @returns 相位列表
 */
function detectAllAspects(planetsA, planetsB) {
    const aspects = [];
